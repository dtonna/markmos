...
void SfxPool::init(ma_engine *eng) noexcept {
    ...
    for (uint8_t i = 0; i < voice_count; ++i) {
        ...
        voices[i].active        = 0;
        voices[i].buffer_inited = 0;
        if (voices[i].sound) {
            memset(voices[i].sound, 0, sizeof(ma_sound));
        }
        if (voices[i].buffer) {
            memset(voices[i].buffer, 0, sizeof(ma_audio_buffer));
        }
    }
}
...
uint8_t SfxPool::register_sound(const char *path) noexcept {
    ...
    if (!decode_file(engine, path, &e.pcm, &e.fmt, &e.channels, &e.sample_rate, &e.frame_count)) {
...
static void voice_release(SfxVoice &v) noexcept {
    if (v.active && v.sound) {
        ma_sound_stop(v.sound);
        ma_sound_uninit(v.sound);
    }
    if (v.buffer_inited && v.buffer) {
        ma_audio_buffer_uninit(static_cast<ma_audio_buffer *>(v.buffer));
        v.buffer_inited = 0;
    }
    v.active   = 0;
...
static bool play_entry(SfxPool &pool, uint8_t id, uint8_t priority, float pitch, float volume_scale) noexcept {
    ...
    for (uint8_t i = 0; i < pool.voice_count; ++i) {
        ...
        if (!v.active && idx == MAX_SFX_VOICES) {
            idx = i;
        }
    }
    ...
    if (idx == MAX_SFX_VOICES) {
        ...
        if (pool.voices[worst].priority <= priority) {
            return false;
        }
        voice_release(pool.voices[worst]);
        idx = worst;
    }
    ...
    // Each voice gets its own buffer wrapping the same PCM so overlapping plays
    // of the same clip do not share a cursor (shared cursor never reaches EOF → sound hangs).
    auto                   *buf  = static_cast<ma_audio_buffer *>(v.buffer);
    ma_audio_buffer_config   bcfg = ma_audio_buffer_config_init(e.fmt, e.channels, e.frame_count, e.pcm, nullptr);
    bcfg.sampleRate              = e.sample_rate;
    if (ma_audio_buffer_init(&bcfg, buf) != MA_SUCCESS) {
        return false;
    }
    v.buffer_inited = 1;

    ma_sound_config scfg = ma_sound_config_init();
    scfg.pDataSource     = buf;
    scfg.flags           = MA_SOUND_FLAG_NO_SPATIALIZATION;

    if (ma_sound_init_ex(pool.engine, &scfg, v.sound) != MA_SUCCESS) {
    ...
    ma_sound_set_volume(v.sound, pool.master_volume * volume_scale);
...
void SfxPool::shutdown() noexcept {
    for (uint8_t i = 0; i < voice_count; ++i) {
        ...
        if (voices[i].buffer) {
            ma_free(voices[i].buffer, nullptr);
...
bool        MusicLayer::is_playing() const noexcept { return track[active_track] && ma_sound_is_playing(track[active_track]); }
...
bool        AudioSystem::init() noexcept {
    ...
    engine = static_cast<ma_engine *>(ma_malloc(sizeof(ma_engine), nullptr));
    if (!engine) {
        return false;
    }
    ...
    if (result != MA_SUCCESS) {
        ...
        if (result != MA_SUCCESS) {
            ...
            ma_free(engine, nullptr);
            engine = nullptr;
            return false;
        }
    }

    MM_LOG("ma_engine initialized. Sample Rate: %u, Channels: %u", ma_engine_get_sample_rate(engine), ma_engine_get_channels(engine));
