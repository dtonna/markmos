...
// ─── Button shape params (CPU-side, uploaded as UBO before flush) ──
struct ButtonParams {
    float border_color_r, border_color_g, border_color_b, border_color_a;
    float icon_color_r, icon_color_g, icon_color_b, icon_color_a;
    float aspect_x, aspect_y; // width/height pair
    float border_width;       // normalized 0..1
    float has_texture;        // 0 = solid fill, 1 = sample texture
};

// Ensure ButtonParams is 48 bytes (6 × float4)
static_assert(sizeof(ButtonParams) == 48, "ButtonParams must be 48 bytes");
...
struct SpriteVertex {
    ...
    uint16_t u, v;    // f16 UV
    uint32_t color;   // RGBA8 fill color
    uint16_t local_x; // f16 local coord (-0.5..+0.5) for SDF rounded rect
    uint16_t local_y;
    uint16_t radius;       // f16 corner radius (normalized 0..0.5)
    uint16_t border_w;     // f16 border width (normalized 0..0.5, 0 = no border)
    uint32_t border_color; // RGBA8 border color (ignored when border_w = 0)
};
...
struct SpriteBatch {
    ...
    float                     corner_r[MAX_SPRITES];     // corner radius (normalized 0..0.5)
    float                     border_w[MAX_SPRITES];     // border width (normalized, 0 = no border)
    uint32_t                  border_color[MAX_SPRITES]; // border RGBA8
    ...
    void add(float x, float y, float sx, float sy, float rot, uint32_t col, uint8_t layer_id, uint16_t tex = 0, float radius = 0.0f, float bw = 0.0f,
             uint32_t bcol = 0) noexcept {
    ...
    void add_frame(float x, float y, float sx, float sy, float rot, uint32_t col, uint8_t layer_id, const SpriteFrame &frame) noexcept {
        if (count >= MAX_SPRITES) {
            return;
        }
        uint16_t i      = count++;
    ...
    void add_frame(float x, float y, float sx, float sy, float rot, uint32_t col, uint8_t layer_id, const SpriteFrame &frame, float radius) noexcept {
        if (count >= MAX_SPRITES) {
            return;
        }
        uint16_t i      = count++;
    ...
    void add_frame(float x, float y, float sx, float sy, float rot, uint32_t col, uint8_t layer_id, const SpriteFrame &frame, float radius, float bw,
                   uint32_t bc) noexcept {
        if (count >= MAX_SPRITES) {
            return;
        }
        uint16_t i      = count++;
    ...
    void add_tile(float x, float y, float sx, float sy, float rot, uint32_t col, uint8_t layer_id, uint16_t tile_col, uint16_t tile_row) noexcept {
        if (count >= MAX_SPRITES) {
            return;
        }
        uint16_t i      = count++;
    ...
    // Add a button shape (same as add() with radius — vertex format is identical)
    // ButtonParams UBO must be bound separately before flush.
    void add_button(float x, float y, float sx, float sy, float rot, uint32_t col, uint8_t layer_id, uint16_t tex = 0, float radius = 0.15f, float bw = 0.0f,
                    uint32_t bcol = 0) noexcept {
        add(x, y, sx, sy, rot, col, layer_id, tex, radius, bw, bcol);
    }

    // Generate vertices into output buffer — returns vertex count
    // Camera: view-projection transform
    uint32_t generate_vertices(SpriteVertex *out_verts, uint32_t max_verts, float cam_x, float cam_y, float cam_scale) const noexcept {
        ...
        for (uint16_t i = 0; i < count && vert_count + VERTS_PER_QUAD <= max_verts; ++i) {
            ...
            float    cr        = corner_r[i];
            float    bw        = border_w[i];
            uint32_t bcol      = border_color[i];
            float    l_scale   = (sx > 0.0f) ? 1.0f / sx : 0.0f;
            float    l_scale_y = (sy > 0.0f) ? 1.0f / sy : 0.0f;
