...
// gold_border.frag — engine port of lab_gold: static border hugging the
// card edge (no travel), breathing alpha. Two layers: persistent base
// (stays on) + pulse overlay (fully fades in/out).
...
void main() {
    ...
    float a = band * outside * (base_level + pulse_level) * params.glow.a * params.misc.y;
